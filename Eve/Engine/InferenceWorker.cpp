...
void InferenceWorker::threadFunc() {
    float hop_accum[kHopSamples];
    size_t hop_fill = 0;

    // Timeout for semaphore wait — allows periodic check of should_stop_.
    const dispatch_time_t timeout = dispatch_time(DISPATCH_TIME_NOW, 50 * NSEC_PER_MSEC);
    (void)timeout; // base value; recomputed each iteration
    ...
    while (!should_stop_.load(std::memory_order_acquire)) {
        ...
        for (;;) {
            ...
            if (!demand_.load(std::memory_order_acquire)) {
                while (input_buffer_->pop(hop_accum, kHopSamples) > 0) {}
                frame_fill_  = 0;
                first_frame_ = true;
            ...
            if (passthrough_.load(std::memory_order_acquire)) {
                const size_t got = input_buffer_->pop(hop_accum, kHopSamples);
                if (got == 0) break;  // no more data — back to waiting
                if (output_buffer_) output_buffer_->push(hop_accum, got);
            ...
            if (first_frame_) {
                ...
                const size_t got    = input_buffer_->pop(hop_accum + hop_fill, needed);
                if (got == 0) break;  // no more data — back to waiting
                hop_fill += got;
